							...
							<div class="w-full">
								<form method="POST" action="{{ route('users.update', ['user' => Auth::user()->id]) }}">
									...
									<div class="w-full">
										...
										<input type="file" name="post_image" class="w-full border-[1.5px] border-form-strokerounded-lg font-medium text-body-color placeholder-body-color outline-none
									...
									<div class="w-full mt-8">
										...
										<input type="text" name="user_website" value="{{ $user->github }}" placeholder=" {{ __('Github') }}" class="w-full border-[1.5px]
											border-form-stroke rounded-lg py-3 px-5 font-medium ext-body-color placeholder-body-color outline-none focus:border-primary
											active:border-primary transition disabled:bg-[#F5F7FD] disabled:cursor-default">
									</div>
